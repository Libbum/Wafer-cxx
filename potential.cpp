...
dcomp potential(int sx,int sy, int sz) 
{
  ...
  r = A*sqrt(dx*dx+dy*dy+dz*dz);
    ...
    case 22: //Mexican Hat
    {
      ...
      gr = NUMX*A/2-(A/2);
      double tx = -((double) gr + 3.0*A) + ((double) sx)*((2.0*((double) gr))/((double) NUMX - 1));
      gr = NUMY*A/2-(A/2);
      double ty = -((double) gr + 3.0*A) + ((double) sy)*((2.0*((double) gr))/((double) NUMY - 1));
      gr = NUMZ*A/2-(A/2);
      double tz = -((double) gr + 3.0*A) + ((double) sz+(nodeID-1)*DISTNUMZ)*((2.0*((double) gr))/((double) NUMZ - 1));
